...
public class Source {
	...
	/**
	 * List of buffers to hold some initial data from the next sound in the
	 * queue.
	 */
	...
	/**
	 * Reads in initial buffers of data from the next sound in the sound
	 * sequence queue, to reduce lag when the transition occurrs. This method
	 * has no effect on non-streaming sources. This method is used internally by
	 * SoundSystem, and it is unlikely that the user will ever need to use it.
	 * 
	...
	public boolean readBuffersFromNextSoundInSequence() {
		...
		synchronized (soundSequenceLock) {
			if (soundSequenceQueue != null && soundSequenceQueue.size() > 0) {
				if (nextCodec != null)
					nextCodec.cleanup();
				nextCodec = SoundSystemConfig.getCodec(soundSequenceQueue
						.get(0).getFilename());
	...
	public boolean stream() {
		...
		if (rawDataStream) {
			...
			for (int i = 0; i < processed; i++) {
						...
						if (SoundSystemConfig.getStreamQueueFormatsMatch()) {
							if (soundSequenceQueue != null
								...
								if (codec != null)
									codec.cleanup();
								filenameURL = soundSequenceQueue.remove(0);
								codec = SoundSystemConfig.getCodec(filenameURL
										.getFilename());
				...
				/*
				 * if( codec.endOfStream() ) { synchronized( soundSequenceLock )
				 * { if( SoundSystemConfig.getStreamQueueFormatsMatch() ) { if(
				 * soundSequenceQueue != null && soundSequenceQueue.size() > 0 )
				 * { if( codec != null ) codec.cleanup(); filenameURL =
				 * soundSequenceQueue.remove( 0 ); codec =
				 * SoundSystemConfig.getCodec( filenameURL.getFilename() );
				 * codec.initialize( filenameURL.getURL() ); return true; } else
				 * if( toLoop ) { codec.initialize( filenameURL.getURL() );
				 * buffer = codec.read(); if( buffer != null ) { if(
				 * buffer.audioData != null ) channel.queueBuffer(
				 * buffer.audioData ); buffer.cleanup(); buffer = null; } } } }
				 * return false; }
				 */
			}
		}
