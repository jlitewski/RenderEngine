...
public class SoundSystemLoader {
	...
	/**
	 * Parses an XML file and executes the commands it contains. If a
	 * SoundSystem instance is specified, it will be used, otherwise this method
	 * will create a SoundSystem instance and return a handle to it if the XML
	 * file contains a CREATE tag. XML tags are NOT case sensitive. Parameters
	 * must be placed inside whichever tag they belong to (nested PARAMETER tags
	 * are not recognized). Parameter values must NOT contain whitespace (this
	 * includes filenames and sourcenames). Parameter values must NOT be inside
	 * quotes. Parameter names are NOT case sensetive, but parameter values ARE
	 * case sensitive. The supported tags and their parameters are listed below.
	 ...
	 * defined, default values will be used instead. <br>
	 * <br>
	 * !-- <br>
	 * &nbsp;&nbsp; XML Comment <br>
	 * <br>
	 ...
	 *            URL to an XML file.
	 * @param s
	 *            Handle to a SoundSystem instance, or null to create one.
	 * @return Handle to SoundSystem, or null if none was initially specified
	 *         and a CREATE command was not found in the XML file.
	 */
			...
			if (command != null && !command.substring(0, 1).equals("/")) {
				...
				if (command.equals("ADDLIBRARY")) {
					...
					try {
						...
						SoundSystemConfig
								.setCodec(commands.get("EXTENSION"), c);
					} catch (ClassNotFoundException cnfe) {
						errorMessage("Unable to set codec plug-in for "
								+ "extension '" + commands.get("EXTENSION")
					...
					if (sourcename.equals("")) {
						...
						if (verbose)
							message("Command:  s.CommandQueue( new "
									...
									+ " ) );", 1);
						s.CommandQueue(new com.paulscode.sound.CommandObject(
								com.paulscode.sound.CommandObject.NEW_SOURCE,
								priority, toStream, toLoop, sourcename,
								new com.paulscode.sound.FilenameURL(filename),
	...
	/**
	 * Sets whether or not to print out messages while loading (default =
	 * false).
	 * 
	 * @param val
	...
	protected static void message(String message, int indent) {
		logger.info("SoundSystemLoader", message, indent);
	}
	...
	protected static void errorMessage(String message) {
		logger.err("SoundSystemLoader", message, 0);
	}
	...
	protected static void warningMessage(String message) {
		logger.warn("SoundSystemLoader", "Warning in class 'SoundSystemLoader': "+ message, 0);
	}
